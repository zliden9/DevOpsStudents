...
```
Полная - Устанавливается как основная ОС и выступает в роли менеджера распределения серверных мощьностей для гостевых ОС.

Паравиртуализация - Устанавлевается на основную ОС и выступает в роли модификатора гостевых ядер для возможности работать поверх основной ОС
и осуществляет контроль за выделением ресурсов от сервера к гостевой ОС.

Виртуализация уровня ОС - Осуществляет контроль за выделением ресурсов от сервера к гостевой ОС, но работает защёт средств самой серверной ОС.
Работает только с гостевыми ОС чьё ядро совместимо с ядром основной ОС.
```
## Задача 2
...
Такой вариант становится самым отказоустойчевым так как можно соберать кластеры и создавать дополнителные методы быстрого
резервирования и развёртывания.

- Различные Java-приложения
Выбор: - виртуализация уровня ОС
При выборе контейнерного подхода мы получаем высокую производительность и быструю расширяемость по мере необходимости.

- Windows системы для использования Бухгалтерским отделом 
Выбор: - паравиртуализация
Так как возникает потребность в подгрузке разных приложений и доступом к различным ресурсам, а также установка
правового справочника и 1С.

- Системы, выполняющие высокопроизводительные расчеты на GPU
Выбор: - виртуализация уровня ОС
Так как данная виртуализация потребляет меньше всех дополнительных ресурсов и работает напрямую с ядром основной ОС.
Альтернатива: - паравиртуализация
Для увеличения вычеслительных мощьностей и размазывания нагрузки, такие сервера можно соберать в кластеры.
Альтернатива: - физические сервера
В зависиммости от поставленной задачи такой вариант также может подойти так как у него самый короткий путь к 
аппаратным мощьностям, а значет самая высокая скорость вычеслений.
...
```
Например можно установить ОС с паравиртуализацией(KVM(ProxMox)) и на нём развернуть виртуалку с докером(виртуализация уровня ОС)
```
-----------------------------------------------------------------
